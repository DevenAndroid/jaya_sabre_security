...
import '../models/is_availible_update_model.dart';

import '../utils/ApiConstant1.dart';

Future<IsAvailableUserUpdateModel> isUserAvailableUserUpdate(available) async {
  var map = <String, dynamic>{};
  map["is_available"] = available;
  ...
  http.Response response = await http.post(
      Uri.parse(ApiUrls1.userAvailabilityUpdate),
      body: jsonEncode(map),
      headers: {
  ...
  print("SREVER UPDATE RESPONSE11 ::${jsonEncode(map)}");
  print("SREVER UPDATE RESPONSE ::${response.body}");

  if (response.statusCode == 200) {
    return IsAvailableUserUpdateModel.fromJson(json.decode(response.body));
  } else {
