...
Future<ModelAssignment> getAssignment() async {
  ...
  final response =
      await http.post(Uri.parse(ApiUrls.assignmentUrl), headers: headers);
  print('assignment response ' + response.body);
  if (response.statusCode == 200) {
    if (ModelAssignment.fromJson(jsonDecode(response.body)).message ==
        "User not Found") {
      SharedPreferences pref = await SharedPreferences.getInstance();
      pref.clear();
    ...
    return ModelAssignment.fromJson(json.decode(response.body));
  } else {
